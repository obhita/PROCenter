/*
    Modified by Eric Roth OBHITA 2014
    ...
    var defaults = {
      ...
      methods = {
          ...
          init: function (element, opts) {
              ...
              if ($.isEmptyObject(settings)) {
                  ...
                  settings.$tipWrapper.on('click.joyride', '.joyride-close-tip', function (e) {
                      e.preventDefault();
                      methods.end(methods.getSettings(this));
          ...
          tip_template: function (settings, opts) {
              ...
              content = $.trim($(opts.li).html()) + '<div class="joyride-buttons-wrapper">' +
                methods.button_prev(settings, opts.index, opts.button_prev_text) +
                methods.button_next(settings, opts.index, opts.button_next_text) + '</div>' +
              ...
              $('.joyride-content-wrapper', $blank).prepend(content);
              ...
              if (settings.$li === undefined || ($.inArray(settings.$li.index(), settings.pauseAfter) === -1)) {
                  ...
                  if (settings.$li.length && settings.$target.length > 0) {
                      ...
                      if (!/body/i.test(settings.$target.selector) && settings.scroll ) {
                          methods.scroll_to(settings, process);
                      } else {
                          process();
                      }
          ...
          hide: function (settings) {
              if (settings.$li == undefined) return;
          ...
          scroll_to: function (settings, callback) {
              var tipOffset = settings.$target.position().top;
              var $scrollContainer = settings.$target.scrollParent();

              if (tipOffset < $scrollContainer.scrollTop() || tipOffset > ($scrollContainer.height() - 100)) {
                  $scrollContainer.stop().animate({
                      scrollTop: tipOffset
                  }, settings.scrollSpeed, callback);
              } else {
                  callback();
              }
          },
          ...
          pos_default: function (settings, init) {
              ...
              if (settings.$target && !/body/i.test(settings.$target.selector)) {
                  ...
                  if (methods.bottom(settings)) {
                      var top = settings.$target.offset().top;
                      var left = settings.$target.offset().left;
          ...
          showModal: function (settings) {
              ...
              if (!settings.$modal_bg) {
                  ...
                  settings.$modal_bg.on('click.joyride', function (e) {
                      e.preventDefault();
                      methods.end(settings);
                  });
              } else {
